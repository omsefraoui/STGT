...
def load_data(filepath):
    ...
    Attendu format:
    - data/sample_data.csv avec colonnes:
      segment_id, month, F1, F2, ..., F17, IRI

    Returns:
        X: (N, T_total, F) - features
        y: (N, T_total) - IRI target
    ...
    # Si format: segment_id, month, features..., IRI
    # Pivot pour obtenir (N, T, F) shape

    # Get unique segments & time steps
...
def prepare_data_multihorizon(X, y, T=12, horizons=[1, 3, 6, 12]):
    """
    Préparer données pour prédictions multi-horizons

    Args:
        X: (N, T_total, F) features
        y: (N, T_total) target values
        T: window length (12 mois)
        horizons: list of prediction horizons

    Returns:
        dict_horizons: {H: {'X_train', 'y_train', 'X_val', 'y_val',
    ...
    for H in horizons:
        ...
        # Créer sliding windows avec horizon H
        for i in range(N):
        ...
        # Standardization (on training set ONLY)
...
def save_multihorizon_data(dict_horizons, output_dir='data/multihorizon/'):
    """Sauver datasets préparés sur disque"""
